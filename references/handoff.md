...
**Each `commit_message` describes its own agent's work product, never the next one's**: `PlanOutput`'s covers the spec file, `BuildOutput`'s the code, `DocumentOutput`'s the write-up. A chain that commits once can use whichever fits; a chain that commits per step needs all three, and reusing one agent's sentence for another's diff is how a commit log starts lying.
...
The user prompt asks for the shape; the type enforces it. They always travel as a pair, which is what lets one agent serve many calls — same system prompt, different user prompt + output type per call site. Output types live in code, never in `mill.config.yaml`.
...
```json
{
  "speccer": {"session_id": "mill-a1b2c3d4-speccer-9f2e",
              "model": "openai-codex/gpt-5.6-terra", "coding_agent": "pi"},
  "builder": {"session_id": "mill-a1b2c3d4-builder-71ac",
              "model": "openai-codex/gpt-5.6-luna", "coding_agent": "pi"}
}
```

This map is the key that lets a later ADW rejoin each agent's **existing context window**. Run `adw_tickets.py --adw-id a1b2c3d4` after `adw_spec.py` and each agent resumes its own session rather than starting cold.

The map records the model each session was created with. If config drift changes an agent's model, that agent starts a **fresh** session and the map is updated — never a bad resume. `agent_sessions` in `mill.db` is the queryable mirror of this file.

**Files are the raw record; the db is the queryable mirror.** Losing `mill.db` loses nothing that can't be rebuilt from `raw_output.jsonl`, `envelope.json`, and `agent_map.json`.
