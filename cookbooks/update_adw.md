...
```python
    with run.phase(PhaseParams(name="review", kind="agent", owner="reviewer",
                               description="Rule on each thing the ticket asked for")) as ph:
        verdict = ph.call(AgentCall(output_type=ReviewOutput, prompt=prompt,
...
A code phase does its work in the block body and logs what it did. The commit phase that closes `adw_run.py` is the pattern:
...
The pattern from `adw_run.py` — always bounded by a module-level constant. The runner is a **code** phase, because the command is known; only repairing it needs an agent:
