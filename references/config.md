...
The full `mill.config.yaml` spec: every field, how defaults merge, and how model / thinking / tools / extensions map onto the coding agent.

It lives at **`adws/adw_mill_config/mill.config.yaml`** — the default path every `adw_*.py` and the justfile resolve, and where `install.py` / `make_config.py` stamp it. Pass `--config <path>` to any ADW (or set `MILL_CONFIG` for the justfile) to run against a different roster.
...
defaults:
  coding_agent: pi
  model: openai-codex/gpt-5.6-luna      # ALWAYS provider/model-id
  thinking: medium
  harness_engineering: []
...
agents:
  - name: speccer
    coding_agent: pi
    model: openai-codex/gpt-5.6-terra     # ALWAYS provider/model-id
    thinking: high
    color: "#34d399"
...
| `model` | string | Model id, always `provider/model-id`. For Pi, anything in its merged catalog — built-in providers plus `~/.pi/agent/models.json`. The starter roster defaults to `openai-codex/gpt-5.6-luna`. |
...
| `protected_files` | list[string] | Paths **no** agent may modify unless it names them in its own `writes`. Default: `adws/adw_modules/`, `adws/adw_mill_config/`, `adws/adw_*.py` — an agent must not be able to edit the machinery that decides whether its work passed. |
...
| Field | Type | Meaning |
|---|---|---|
| `db` | path | SQLite trace db. `tracer.py` writes it directly; the visualizer polls it. Default `adws/adw_data/mill.db`. |
...
```
agent 'architect': model pattern 'gemini-3.6-flash' is ambiguous:
...
```yaml
defaults:
  protected_files: [adws/adw_modules/, adws/adw_mill_config/, "adws/adw_*.py"]

agents:
  - name: builder      # no `writes` key -> unrestricted, minus protected_files
  - name: architect
    writes: []         # no repo writes; its design still lands in context_handoff/
  - name: speccer
    writes: [.scratch/]
  - name: gatekeeper
    writes: [tests/acceptance/]   # naming a protected path is what unlocks it
```
