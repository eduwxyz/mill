"""Concrete data types for the MILL ADW system.
...
class ResearchOutput(EnvelopeBase):
    """One independent research head.

    Isolation is the whole point: two of these run without seeing each other,
    and where they disagree is where a real decision lives. A head that saw the
    other's answer would converge on it, and convergence is worth nothing here.
    """

    approach: str = ""                                    # what you would do, concretely, in THIS repo
    rejected: str = ""                                    # the alternative you considered and what killed it
    risks: list[str] = Field(default_factory=list)        # where it breaks, specifically
    out_of_scope: list[str] = Field(default_factory=list)
    engineer_questions: list[str] = Field(default_factory=list)


class Divergence(BaseModel):
    """One place the two heads could not both be right."""

    question: str                                         # the disagreement, in one sentence
    position_a: str
    position_b: str
    recommendation: str                                   # which side, or a third way
    because: str                                          # what KILLED the other side — evidence, not taste


class AdrCandidate(BaseModel):
    """A decision that MIGHT deserve an Architecture Decision Record.

    Candidate, never record: the bar exists to REFUSE, and most decisions fail
    it. An empty list is the expected result on most rounds — a bad ADR is
    worse than none, because it records trivia and erodes the habit of reading
    the ones that matter. The engineer signs off; nothing is written from here.

    All three fields must hold. If any one of them takes effort to write, that
    is the answer: it does not pass.
    """

    decision: str                                         # the decision, in one sentence
    hard_to_reverse: str                                  # what undoing it costs — real work, not annoyance
    surprising: str                                       # what a competent reader would expect instead
    alternatives: str                                     # the options that were genuinely viable, not strawmen


class ArchitectOutput(EnvelopeBase):
    """The proposal, plus what remains the engineer's to answer.

    Named `architect` and not `synthesizer` on purpose: it decides. But a
    recommendation can anchor the engineer onto a bad path, so every one of
    them must carry what killed the alternative — an architect that cannot
    write that sentence did not examine it.
    """

    design: str = ""                                      # the proposed approach, in prose
    agreements: list[str] = Field(default_factory=list)   # cheap; usually both read the same code
    divergences: list[Divergence] = Field(default_factory=list)
    engineer_calls: list[str] = Field(default_factory=list)   # business, taste, priority — never the architect's
    default_path: str = ""                                # what happens if the engineer decides nothing
    adr_candidates: list[AdrCandidate] = Field(default_factory=list)   # usually empty, and that is correct


class SpecOutput(EnvelopeBase):
    """The spec: WHAT is being built and WHY, in the requester's language.

    `seams` is called out of the prose on purpose. Where a feature gets tested
    is the decision that shapes everything downstream, and burying it inside a
    markdown section makes it invisible to the ticket workflow that needs it.
    """

    spec_path: str = ""
    seams: list[str] = Field(default_factory=list)        # where it gets tested, and why there
    open_questions: list[str] = Field(default_factory=list)


class Ticket(BaseModel):
    """One tracer bullet: a narrow but COMPLETE path through every layer.

    Vertical, never horizontal. A slice that delivers "the schema" is not a
    ticket — nobody can demo it and nothing proves it works. A slice that
    delivers "a user can see yesterday's total" crosses schema, aggregation and
    UI, and either works or does not.
    """

    number: int                                           # 1-based, in dependency order: blockers first
    title: str
    delivers: str                                         # the end-to-end behaviour, from the user's side
    blocked_by: list[int] = Field(default_factory=list)   # empty = can start immediately
    acceptance: list[str] = Field(default_factory=list)
    path: str = ""                                        # where the ticket file landed


class TicketsOutput(EnvelopeBase):
    """The breakdown, plus what can start right now."""

    tickets: list[Ticket] = Field(default_factory=list)
    frontier: list[int] = Field(default_factory=list)     # numbers with no unmet blockers
    prefactors: list[int] = Field(default_factory=list)   # slices that only make the next change easy


class CriterionOutput(EnvelopeBase):
    """An executable acceptance criterion, plus how to run it.

    The command travels WITH the script because that is what keeps the factory
    off any one language: `["bun", "run", ...]`, `["python3", "-m", "unittest", ...]`,
    `["go", "test", "./..."]`. Code only has to run it and read the exit status.

    Written before a line of implementation exists, which is not a ceremony:
    a criterion written afterwards can see the code, and what it can see it
    will test. This one cannot, so it has nothing to test but the behaviour
    that was asked for.
    """

    script_path: str = ""
    command: list[str] = Field(default_factory=list)      # argv; non-zero exit means failed
    covers: list[str] = Field(default_factory=list)       # the ticket checkboxes made executable
    uncovered: list[str] = Field(default_factory=list)    # checkboxes it could NOT make executable, and why
    #: The REAL data it opened before writing the checks.
    #:
    #: Empty, on a ticket that talks about existing sources, is the warning sign:
    #: the criterion was written against an imagined world. That is how an
    #: importer passed with full marks against three fixtures while reading zero
    #: of the 171 real files sitting on disk.
    grounded_in: list[str] = Field(default_factory=list)


class ShapeFinding(BaseModel):
    """Something you can only see by looking at the WHOLE feature."""

    what: str                                             # what is crooked, in one sentence
    where: list[str] = Field(default_factory=list)        # files, with a line when useful
    why_it_matters: str = ""                              # what it costs three months from now
    severity: Literal["high", "medium", "low"] = "medium"
    suggested_ticket: str = ""                            # the "What to build" of a future ticket


class ShapeOutput(EnvelopeBase):
    """The reading of the feature as a whole — never per ticket.

    Kept apart from `ReviewOutput` on purpose: that one asks "is this what the
    ticket asked for?", which is answerable ticket by ticket. This one asks "what
    came out crooked once you see it all together?", which NO ticket can see —
    because each one was right on its own.
    """

    report_path: str = ""
    findings: list[ShapeFinding] = Field(default_factory=list)
    what_held_up: list[str] = Field(default_factory=list)   # what came out well, and why
    verdict: str = ""                                       # one honest sentence about the state
...
class ConfigDefaults(BaseModel):
    ...
    protected_files: list[str] = Field(default_factory=lambda: [
        "adws/adw_modules/", "adws/adw_mill_config/", "adws/adw_*.py",
    ])
    data_dir: str = "adws/adw_data"


class ObservabilityConfig(BaseModel):
    db: str = "adws/adw_data/mill.db"
    poll_ms: int = 500
...
class UsageBreakdown(BaseModel):
    ...
    #: How much of `total_cost` was ESTIMATED from tokens instead of charged by
    #: the provider. A subscription (the Claude bridge) returns zero cost because
    #: the turn generates no invoice; without the estimate the factory would lose
    #: the only measurement comparable across runs. > 0 means: compare freely, but
    #: do not add it to a bill.
    estimated_cost: float = 0.0

    def add_turn(self, usage: dict, total_tokens: int, estimated: float = 0.0) -> None:
        ...
        cobrado = cost.get("total") or 0.0
        self.total_cost += cobrado or estimated
        if not cobrado:
...
class PiResult(BaseModel):
    ...
    #: How much of `cost` is ESTIMATED rather than charged. A subscription
    #: provider (the Claude bridge) returns cost=0 because the turn generates no
    #: invoice; the estimate comes from tokens and exists to COMPARE runs, never
    #: to add to a bill. A run with this > 0 has a total that is not an invoice.
    estimated_cost: float = 0.0
