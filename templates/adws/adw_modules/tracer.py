...
Files are the raw record; mill.db is the queryable mirror the UI polls.
...
class _Rows(list):
    """What `execute` returns: already materialized, and still cursor-shaped.

    The original cursor would be consumed OUTSIDE the lock, which would hand the
    race back through the side door. Materializing inside it costs nothing here —
    the tracer's reads are one row or a handful of columns.
    """
...
class _Locked:
    """A SQLite connection that survives being used from more than one thread."""
    ...
    def execute(self, *args, **kwargs) -> _Rows:
        with self._lock:
            ...
            try:
                ...
                return _Rows()          # INSERT/UPDATE return no rows
...
class Tracer:
    def __init__(self, db_path: str | Path, events_jsonl: str | Path):
        ...
        # `check_same_thread=False` + a lock: the trace now survives phases that
        # run in parallel. Without it Python REFUSES the connection coming from
        # another thread, and the first write from a concurrent phase blows up —
        # that is what held the two research heads in sequence.
        #
        # The lock is on the CONNECTION, not on each method: there are ~15 places
        # calling `self.conn.execute` directly, and locking in one place is the
        # difference between a single point of truth and fifteen chances to forget
        # one.
        self.conn = _Locked(sqlite3.connect(self.db_path, isolation_level=None,
    ...
    def _migrate(self) -> None:
        """Additive column migrations, so a db from an older MILL still opens."""
