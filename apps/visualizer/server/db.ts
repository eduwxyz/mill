/**
 * SQLite reader over a target repo's mill.db.
 *
...
/**
 * Resolve the db path: --db arg wins, then MILL_DB, then <cwd>/adws/adw_data/mill.db.
...
export class MillDb {
   ...
   * `adws/adw_data/mill.db`), so deriving it as a sibling of the db file keeps
  ...
  constructor(path: string) {
    if (!existsSync(path)) {
      throw new Error(
        `mill.db not found at ${path}\n` +
          `Point the visualizer at a target repo: --db <path> or MILL_DB=<path>, ` +
