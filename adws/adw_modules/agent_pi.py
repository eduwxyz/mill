...
def resolve_model(pattern: str) -> tuple[str, str]:
    ...
    that same merged view lets MILL target direct providers such as
...
def context_window(provider: str, model_id: str) -> int:
    ...
    # models.json only exists once someone REGISTERS a custom model. Anyone
    # running only the built-in providers (anthropic, openai-codex) never has
    # that file — and pi works perfectly without it. Reading it without checking
    # brought the factory down on the FIRST agent, with FileNotFoundError, on a
    # path that exists only to discover a context-window number. The
    # `pi --list-models` catalog, just below, already answers that.
    try:
        ...
        for line in process.stdout:
            ...
            if event.get("type") == "message_end":
                ...
                if message.get("role") == "assistant":
                    ...
                    cobrado = (usage.get("cost", {}) or {}).get("total", 0.0) or 0.0
                    estimado = 0.0
                    if cobrado:
                        result.cost += cobrado
                    else:
                        # A subscription provider (the Claude bridge) returns
                        # cost=0: that turn generated no invoice. Zeroing the
                        # number would erase the only comparable measurement
                        # there is — which ticket weighed more, which agent
                        # burns through budget. We estimate from tokens, and the
                        # result is NOTIONAL, never billed.
                        estimated = pricing.estimate(model_id, usage)
                        result.cost += estimated
                        result.estimated_cost += estimado
                    result.usage.add_turn(usage, turn, estimado)
            if on_event:
                on_event(event)
