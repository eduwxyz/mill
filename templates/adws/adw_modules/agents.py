...
from .data_types import (AgentCall, AgentConfig, EnvelopeBase, EventRecord,
                         GateCheck, GateReport, Phase, PiRequest, MILLConfig,
                         UsageBreakdown)
...
def load_config(path: str = "adws/adw_mill_config/mill.config.yaml") -> MILLConfig:
    ...
    return _redirect_runtime(MILLConfig(**raw))


def _redirect_runtime(cfg: MILLConfig) -> MILLConfig:
    """Lets the trace leave the cwd when MILL_DB/MILL_DATA_DIR say so.

    `db` and `data_dir` are RELATIVE paths, resolved against the directory the
    process runs from. That is right in normal use and destructive when the run
    happens inside a worktree: the trace is born in there, and the worktree is
    deleted the moment the ticket integrates. The work survives (it is in the
    commit); the EVIDENCE of how it was made does not.

    With these two variables, the loop points the children at the main
    repository's database — one single trace, which the UI can see, and which
    goes on existing after the checkout disappears.
    """
    db = os.environ.get("MILL_DB")
    data_dir = os.environ.get("MILL_DATA_DIR")
    if db:
        cfg.observability.db = db
    if data_dir:
        cfg.defaults.data_dir = data_dir
    return cfg


def resolve(cfg: MILLConfig, name: str) -> AgentConfig:
...
def validate(cfg: MILLConfig, required: list[str]) -> None:
...
def execute(run, phase: Phase, call: AgentCall) -> EnvelopeBase:
    ...
    if touched:
        ...
        # Accumulated on the run: it is what makes it possible to commit ONLY
        # what this execution changed, instead of sweeping the whole tree with
        # `git add -A`.
        for path in touched:
            if path not in run.touched:
                run.touched.append(path)
...
def _agent_session_id(run, agent: AgentConfig) -> str:
    ...
    return f"mill-{run.adw_id}-{agent.name}-{new_id(4)}"
...
def _event_forwarder(run, phase: Phase, agent_name: str):
    ...
    def forward(event: dict) -> None:
        ...
        if record is None:
            return
        run.console.tool_call(agent_name, record.get("label", ""))
