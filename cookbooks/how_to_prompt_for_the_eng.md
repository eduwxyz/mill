...
Read this **before every ADW launch**. The prompt you pass is what the whole chain reads: the speccer specs from it, the slicer slices from it, the gatekeeper writes the criterion from it, the builder builds from it, the reviewer judges against it. Your prompt might run through 10s or 100s of agents. A sloppy prompt is not a small tax; it is paid again by every agent in the chain.
...
- **Do not write the design.** Your prompt says WHAT and DONE MEANS. HOW belongs to the architect and the speccer — unless the engineer specified how, and then you carry it word for word.
