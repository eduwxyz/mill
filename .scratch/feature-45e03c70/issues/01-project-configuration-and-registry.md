...
- [ ] Regression coverage proves that legacy resolution is byte-for-byte unchanged when no project configuration is selected, including a `/private/...` database path on macOS; project configuration must not normalize that legacy result.
