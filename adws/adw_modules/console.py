...
class Console:
    ...
    def tool_call(self, agent_name: str, label: str) -> None:
        """One line per tool the agent actually used, while it is using it.

        The trace already kept this — arguments, result, duration — but only the
        visualizer showed it, and the visualizer is per project and has to be
        running. Anyone in the terminal saw a phase start and a phase end, with
        minutes of silence in between, not knowing whether the agent was working
        or stuck.

        One line per call, with the agent's name in front: when two heads run in
        parallel the lines interleave, and the prefix is the only thing that says
        who did what.
        """
        self._emit(f"    [dim]▸ {escape(agent_name)}  {escape(_clip(label, 100))}[/dim]")
