---
name: mill
description: Mill — deploy and operate repeatable agents+code workflows (ADWs) in any codebase. Use when the user says /mill install, wants to create/run/update an ADW, manage the agent roster in mill.config.yaml, or observe running agent workflows. Keywords - mill, software factory, ADW, AI developer workflow, agent pipeline, install factory.
...
1. Read [cookbooks/mill_overview.md](cookbooks/mill_overview.md) — the system map.
...
| ADW | Chain | Use when |
|---|---|---|
| adw_research_fusion | researcher_a ∥ researcher_b → architect | the idea is still vague; the divergence is the product |
| adw_run | gatekeeper → red → builder → verify → reviewer → commit | one ticket, behind a criterion that had to fail first |
```
...
- Observe by querying `adws/adw_data/mill.db` (WAL — reads never block writers) **when observing is the task**. This is a capability, not a startup step: query it to follow a run you launched or one the engineer asked about, never to volunteer a status report nobody requested.
...
| Request | Cookbook |
|---|---|
| `/mill install`, set up the factory in this repo | [cookbooks/install.md](cookbooks/install.md) |
...
Vocabulary, when a term here is unfamiliar: [CONTEXT.md](CONTEXT.md) — *criterion*, *red baseline*, *weak gate*, *real anchor*, *frontier*, *wave*, *envelope*, *notional cost*. It is a glossary, not a routing step: read the entry you need, never the file up front.
...
Pi coding agent only (`coding_agent: pi`), default model `openai-codex/gpt-5.6-luna`, thinking `medium`. `claude_code` is schema-valid but stubbed until v2. The visualizer app lives in `apps/visualizer/` and is pointed at a project's db (`MILL_DB=<repo>/adws/adw_data/mill.db bun run dev:all`); sqlite queries are the headless equivalent and always work.
