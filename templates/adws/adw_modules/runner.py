...
class Run:
    def __init__(self, cfg, adw_id: str, tracer, engineer: str):
        ...
        # Every path THIS run's agents changed, in order. It is what the commit
        # uses to avoid sweeping the whole tree — see git_helper.commit_paths.
        # In __init__ and not on the class: a mutable list as a class attribute is
        # shared between instances, and two runs would bleed into each other.
    ...
    def phase(self, params: PhaseParams):
        ...
        try:
            ...
            # Some exceptions have an empty str() — StopIteration is the classic
            # one — and then the trace recorded `error: ""`. The type always
            # exists, and on its own it already points at the place.
            phase.error = (str(error) or f"{type(error).__name__} (no message)")[:1000]
