...
# The roster's agents mapped to the envelope each one returns. An agent that is
# not here falls back to GenericOutput — right for a one-off ask, wrong the
# moment the next phase needs a named field out of it.
OUTPUT_TYPES = {"researcher_a": "ResearchOutput", "researcher_b": "ResearchOutput",
                "architect": "ArchitectOutput", "speccer": "SpecOutput",
                "slicer": "TicketsOutput", "gatekeeper": "CriterionOutput",
                "builder": "BuildOutput", "reviewer": "ReviewOutput",
                "critic": "ShapeOutput"}
...
Usage:
    uv run adws/adw_{name}.py "<prompt or path/to/prompt.md>" [--config adws/adw_mill_config/mill.config.yaml] [--adw-id a1b2c3d4]
