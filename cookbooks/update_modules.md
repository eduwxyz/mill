...
| Module | Owns |
|---|---|
| `data_types.py` | Every Pydantic model: `AgentCall`, `PhaseParams`, `Phase`, `EnvelopeBase` + one output type per agent call, the config models (`AgentConfig`, `MILLConfig`), `EventRecord`, and `PiRequest`/`PiResult` |
...
| `tracer.py` | append JSONL **and** insert every event into `mill.db` as it happens |
...
Modules report through `run.console` — never a bare `print()`. Each console method prints a rich line **and** writes it to `mill.db` as a `log` event with payload `{message, level}`, both from one `_emit` helper, so the terminal narrative and the swim-lane UI can't drift. New output means a new method on `Console`, not a print at the call site.
...
Run the factory suite — `just test` — and then the cheapest real ADW, `just investigate "<a one-line question>"`, since every module change rides the same path a real run does.
