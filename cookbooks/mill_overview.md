...
The system map the orchestrator reads on startup — what MILL is, how a stamped repo is laid out, and which cookbook to load next.

## What MILL is

Mill builds repeatable **agents plus code** workflows. Deterministic Python (an ADW script) owns sequencing, retries, and acceptance; agents are bounded nodes inside that graph. Agent proposes, code disposes.
...
adws/
├── adw_mill_config/
│   └── mill.config.yaml         the agent roster — one agent, one prompt, one purpose
├── adw_research_fusion.py       two isolated researchers + an architect who decides
├── adw_spec.py                  the decided design, in seven sections
├── adw_tickets.py               vertical slices with an explicit blocking graph
├── adw_run.py                   ONE ticket: red criterion → build → verify → review → commit
├── adw_frontier.py              the waves, AFK — parallel within one, integrate between
├── adw_review.py                read the WHOLE feature and name what came out crooked
...
│   ├── criterion.py             runs the acceptance criterion and says what its exit status MEANT
│   ├── quality.py, detect.py    the project's own checks, detected rather than memorized
│   ├── frontier.py              the blocking graph: what can start now, and what is complete
│   ├── worktree.py              one isolated checkout per ticket; integration is explicit
│   ├── permissions.py           `writes:` enforced after every agent call, unauthorized changes rolled back
│   ├── changes.py               git diff vs a resolved base → ChangeSet → envelope for the critic
│   ├── pricing.py               notional cost when the provider bills a subscription, not tokens
│   └── prompts.py, session.py, tracer.py, console.py, git_helper.py, naming.py, utils.py
└── adw_data/
    ├── prompt_engineering/{agent}/{system.md,user.md}   tracked — edit prompts HERE, never in the skill
    │       researcher · architect · speccer · slicer · gatekeeper · builder · reviewer · critic
    ├── harness_engineering/                             tracked — pi extensions the roster can load
    ...
    └── mill.db                  gitignored SQLite trace db the visualizer polls
```

**v1 runs Pi only.** `coding_agent: pi`, default model `openai-codex/gpt-5.6-luna`, thinking `medium`. `claude_code` is specced in the config and stubbed in the interface — it lands in v2.
...
- **code** — deterministic steps that stand alone (git branch, git commit, run the criterion). Never buried inside an agent phase.
...
```bash
uv run adws/adw_research_fusion.py "a dashboard that shows token spend per feature"
uv run adws/adw_spec.py "<the engineer's answers>" --title spend-dashboard --adw-id a1b2c3d4
```
...
The `justfile` wraps each of these — `just research`, `just investigate`, `just spec`, `just tickets`, `just run`, `just frontier`, `just review` — and is the form the engineer usually types.
...
| Generate `mill.config.yaml` | `create_config.md` |
