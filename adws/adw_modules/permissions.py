...
Two keys drive it, both in mill.config.yaml:
...
from .data_types import AgentConfig, MILLConfig
...
def snapshot(run) -> dict[str, str]:
    ...
    for path in _git(["ls-files", "--others", "--exclude-standard"],
        ...
        if not path:
            continue
        # SIZE AND MTIME, not just the name.
        #
        # A fixed `"untracked"` made the REWRITE of an untracked file invisible:
        # before and after, the fingerprint was the same, and the run concluded
        # nothing had changed. In a new repository — where EVERYTHING is
        # untracked — that meant a ticket's second attempt always ended in
        # "nothing to commit", with the work done on disk and no commit.
        try:
            info = (Path(run.repo_root) / path).stat()
            fingerprints[path] = f"untracked,{info.st_size},{info.st_mtime_ns}"
        except OSError:
...
def always_writable(cfg: MILLConfig) -> list[str]:
...
def permitted(path: str, agent: AgentConfig, cfg: MILLConfig) -> bool:
